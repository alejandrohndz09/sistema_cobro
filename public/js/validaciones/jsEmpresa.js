...
function agregar() {
    ...
    $('#method').val('POST');
    $('#sucursalForm').attr('action', '');
    $('#modalForm').modal('show');
}

function editar(idSucursal) {
    $.get('/opciones/empresa/' + idSucursal + '/edit', function (obj) {
        const errorSpans = document.querySelectorAll('span.text-danger');
        errorSpans.forEach(function (span) {
            span.innerHTML = '';
        });
        ...
        $('#method').val('PUT');
        $('#sucursalForm').attr('action', '/opciones/empresa/' + idSucursal);
        $('#modalForm').modal('show');
    });
}

function eliminar(idSucursal) {
    $('#confirmarForm').attr('action', '/opciones/empresa/' + idSucursal);
    $('#methodS').val('Delete');
    $('#dialogo').text('Está a punto de eliminar permanentemente el registro. ¿Desea continuar?');
}

function baja(idSucursal) {
    $('#confirmarForm').attr('action', '/opciones/empresa/baja/' + idSucursal);
    $('#methodS').val('get');
...
function alta(idSucursal) {
    $.ajax({
        url: '/opciones/empresa/alta/' + idSucursal,
        method: 'get',
        success: function (response) {
            Toast.fire({
                icon: response.type,
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            originalData = data.map(c => {
                ...
                if (c.estado == 1) {
                    acciones = `
                        <a role="button" data-bs-toggle="modal" data-bs-target="#modalForm" data-id="${c.idSucursal}" data-bs-tt="tooltip" data-bs-original-title="Editar" class="btnEditar me-3">
